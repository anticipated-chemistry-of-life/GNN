...
def load_input_data(path):
    ...
    if data.columns.isnull().any():
        raise ValueError(
            "The input DataFrame has unnamed columns ! Columns should be named."
        )
...
def check_which_model_to_use(G: nx.DiGraph, data: pd.DataFrame):
    ...
    if data.columns[0] not in valid_names:
        raise ValueError(
            "First column must be named either : 'molecule' or 'species' !"
        )
    if data.columns[1] not in valid_names:
        raise ValueError(
            "Second column must be named either : 'molecule' or 'species' !"
        )
    ...
    if data.columns[0] == "molecule":
        m_to_s = data_copy[data_copy["model"] == "m_to_s"][
            ["molecule", "species"]
        ].values
        s_to_m = data_copy[data_copy["model"] == "s_to_m"][
            ["molecule", "species"]
        ].values
        both_unknown = data_copy[data_copy["model"] == "both_unknown"][
            ["molecule", "species"]
        ].values
        both_known = data_copy[data_copy["model"] == "both_known"][
            ["molecule", "species"]
        ].values
...
def add_nodes_to_graph(G: nx.DiGraph, data: pd.DataFrame) -> nx.DiGraph:
    ...
    if len(data.columns) != 3:
        raise IndexError(
            "Input data must have 3 columns ! Mol, Species, and Model. Something went wrong in the previous steps"
        )
...
def nx_to_stellargraph(
    g: nx.DiGraph, molecule_features: pd.DataFrame, species_features: pd.DataFrame
) -> stellargraph.core.graph.StellarDiGraph:
    if (
        set(g.nodes()) - set(molecule_features.index) - set(species_features.index)
        != set()
    ):
        print(
            set(g.nodes()) - set(molecule_features.index) - set(species_features.index)
        ...
        raise Exception(
            "Some nodes do not have features ! Please check your graph or your features."
        )
    ...
    G = StellarGraph.from_networkx(
        g, node_features={"species": species_feat, "molecule": mol_feat}
    )
...
def create_flow(
    graph: stellargraph.core.graph.StellarDiGraph,
    data: pd.DataFrame,
    array: np.ndarray,
    ...
    if data.columns[0] == unknown_node:
        flow = HinSAGELinkGenerator(
            ...
            batch_size=1024,
            num_samples=[3, 1],
            head_node_types=[data.columns[0], data.columns[1]],
        ).flow(array, np.ones(len(array)).reshape(-1, 1))
    ...
    # head node types order
        ...
        flow = HinSAGELinkGenerator(
            ...
            batch_size=1024,
            num_samples=[3, 1],
            head_node_types=[data.columns[1], data.columns[0]],
        ).flow(array[:, ::-1], np.ones(len(array)).reshape(-1, 1))
...
def _predict_using_both_models(
    model_m_to_s,
    model_s_to_m,
    flow_m: stellargraph.mapper.sequences.LinkSequence,
    flow_s: stellargraph.mapper.sequences.LinkSequence,
) -> np.ndarray:
    ...
    if len(a) != len(b):
        raise IndexError(
            f"Forward is of length {len(a)} and backward is of length {len(b)}. They should be the same."
        )
...
def predict(
    ...
    (
        missing_mols_classyfire,
        missing_mols_fingerprint,
        missing_sp_taxo,
    ) = get_missing_features(data_convert, graph, n_cpus=n_cpus)
    ...
    if len(species_features.columns) != 69:
        raise NotImplementedError(
            f"The model has been trained on 69 features for species, with your species there are now {len(species_features.columns)} features. This is not supported. Please give less species as input."
        )
    ...
    if missing_mols_classyfire is not None:
        ...
        mols_feat_fingerprint = pd.concat(
            [molecule_fingerprint, missing_mols_fingerprint]
        )
    else:
    ...
    molecule_features = mols_feat.merge(
        mols_feat_fingerprint, left_index=True, right_index=True
    )

    del mols_feat, mols_feat_fingerprint
    if len(molecule_features.columns) != 155:
        raise NotImplementedError(
            f"The model has been trained on 155 features for molecules, with your molecules there are now {len(molecule_features.columns)} features. This is not supported. Please give less molecules as input."
        )

    # first check which model should be used for each row
    print("Checking which model should be used for each row...")
    data_out, m_to_s, s_to_m, both_unknown, both_known = check_which_model_to_use(
        graph, data_convert
    )
    ...
    graph_stellar = nx_to_stellargraph(
        graph_with_nodes, molecule_features, species_features
    )
    ...
    if both_unknown.size != 0:
        ...
        flow_m_both_unknown = create_flow(
            graph_stellar, data_convert, both_unknown, "molecule"
        )
        flow_s_both_unknown = create_flow(
            graph_stellar, data_convert, both_unknown, "species"
        )
        out_both_unknown = _predict_using_both_models(
            model_m_to_s, model_s_to_m, flow_m_both_unknown, flow_s_both_unknown
        )
    ...
    if both_known.size != 0:
        ...
        flow_m_both_known = create_flow(
            graph_stellar, data_convert, both_known, "molecule"
        )
        flow_s_both_known = create_flow(
            graph_stellar, data_convert, both_known, "species"
        )
        out_both_known = _predict_using_both_models(
            model_m_to_s, model_s_to_m, flow_m_both_known, flow_s_both_known
        )
    ...
    if data_convert.columns[0] == "molecule":
        out_df = pd.DataFrame(
            np.vstack((m_to_s, s_to_m, both_unknown, both_known)),
