...
def _filter_df(df: pd.DataFrame, compound: str) -> pd.DataFrame:
    ...
    df_filtered = df[df['ClassifiedResults'].str.split(': ').str[0].isin(classes)].copy()
    
    # Split 'ClassifiedResults' into two separate columns
    df_filtered[['Classification', 'Result']] = df_filtered['ClassifiedResults'].str.split(': ', expand=True)
